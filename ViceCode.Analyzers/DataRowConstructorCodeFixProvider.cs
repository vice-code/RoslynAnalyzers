...
namespace ViceCode.Analyzers
{
    ...
    public class DataRowConstructorCodeFixProvider : CodeFixProvider
    {
        ...
        private async Task<Document> CreateDataRowConstructor(Document document, ClassDeclarationSyntax classDeclaration, CancellationToken ct)
        {
            ...
            {
                statements.Add(SyntaxFactory.ExpressionStatement(CreatePropertyAssigmentExpression(parameter, property)));
            }
            ...
            var newClassDexlaration = classDeclaration.InsertNodesBefore(classDeclaration.Members.FirstOrDefault(), new[] { constructor.WithLeadingTrivia(properties.First().GetLeadingTrivia()) });
